        ...
        try {
            ...
            if (currentStep === 1) {
                ...
                const { error: incomeError } = await supabase.from('user_preferences').update({
                    ...
                    program_preference: 'prouni',   // [NEW] Default for current season
                    university_preference: 'privada', // [NEW] Default for current season
                                ...
                                <div className="flex p-1 bg-gray-100/50 rounded-xl">
                                    {AVAILABLE_YEARS.map(year => (
                                        <button
                                            ...
                                            className={`flex-1 py-1.5 text-sm font-medium rounded-lg transition-all ${selectedYear === year && !didNotTakeEnem
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {QUOTA_OPTIONS.map((opt) => (
                                    <button
                                        ...
                                        disabled={noQuota}
                                        className={`relative p-3 rounded-xl border-2 text-left transition-all flex items-center justify-between ${selectedQuotas.includes(opt.id)
                                    ...
                                    >
                                        <div className="flex items-center w-full min-w-0">
                                            <div className={`shrink-0 w-5 h-5 rounded-full border-2 mr-3 flex items-center justify-center transition-colors ${selectedQuotas.includes(opt.id) ? 'border-[#024F86] bg-[#024F86]' : 'border-gray-300'
                                                }`}>
